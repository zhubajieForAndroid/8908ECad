...
public class FillingThreeActivityDemo extends BaseActivity implements View.OnTouchListener, View.OnClickListener {
    ...
    private boolean isAddTooFast = false;   //是否加注过快
    private boolean isAddBack = false;   //是否回收过快
    private boolean isSureTwoClose = true;
                ...
                if (isSuccess) {
                    ...
                    MyApplication.getmHandler().postDelayed(new Runnable() {
                        ...
                        public void run() {
                            ...
                            currentState = 6;
                            //打开2#4#触点
                            int[] arr = {0x2a, 0x06, 0x01, 0x0a, 0x27, 0x23};
    ...
    private class CheckliquidTask extends TimerTask {
        @Override
        public void run() {
            String stirNumber = mSp.getString("stirNumber", "");
            final float mOldNumber = Integer.parseInt(stirNumber, 16);
            final float currentNumber = Integer.parseInt(mRiseNumbwe, 16);
            if (currentNumber < (mOldNumber - 500)) {           //加注过快,关闭加注
                if (isSureTwoClose) {
                    isAddTooFast = true;
                    currentState = 8;
                    isSureTwoClose = false;
                    //关闭2#触点,开启4#
                    int[] arr = {0x2a, 0x06, 0x01, 0x08, 0x25, 0x23};
                    SendUtil.sendMessage(arr, MyApplication.getOutputStream());
                }
            } else if (currentNumber > (mOldNumber + 500)) {      //回收过块,关闭回收
                if (isFourOpen) {
                    isAddBack = true;
                    currentState = 9;
                    isFourOpen = false;
            ...
            } else {                                             //回收和加注的差值没有超过500ml
                if (currentNumber > mOldNumber) {
                    sendResult(mOldNumber, currentNumber);
                } else {
                    sendResult(currentNumber, mOldNumber);
                }
            }
        }
    }

    private void sendResult(float mOldNumber, float currentNumber) {
        if ((currentNumber - mOldNumber) <= 10) {
            if (isAddTooFast) {
                isAddTooFast = false;
                currentState = 10;
                //打开2#触点,打开4#
    ...
    private void startCheckliquidTask() {
        ...
        if (mCheckliquidTask == null) {
            ...
            mChickliquidTimer.schedule(mCheckliquidTask, 1000, 1000);
        }
    }
...
/*--------------------加注的时候检查液体的任务开始---------------------*/
    ...
    private class CheckTimeTask extends TimerTask {
        ...
        public void run() {
            ...
            final float currentNumber = Integer.parseInt(mRiseNumbwe, 16);
            if ((parseInt - currentNumber) >= 1500) {
                currentState = 11;
                SendUtil.closeAll();
            }
    ...
    private void startCheckTimeTask() {
        ...
        if (mCheckTimeTask == null) {
            ...
            mTimer.scheduleAtFixedRate(mCheckTimeTask, 1000, 1000);
        }
    }
