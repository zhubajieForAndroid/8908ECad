...
public class Formaldehyde extends RelativeLayout {
    LineChart mLineChart;
    private ChartManage mManage;
    List<Entry> data1 = new ArrayList<>();
    List<Entry> data2 = new ArrayList<>();
    public Formaldehyde(Context context) {
        this(context, null);
    }

    public Formaldehyde(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
    ...
    private void init(Context context) {
        ...
        mManage = new ChartManage(mLineChart, context,1);
    }
    ...
    public void setData(CurverBean.ResponseBean responseBean) {
        ...
        for (int i = 0; i < before.size(); i++) {
            ...
            Entry entry = new Entry(i, Float.parseFloat(beforeBean.getFormaldehyde()));
            data1.add(entry);
        }
        for (int i = 0; i < after.size(); i++) {
            CurverBean.ResponseBean.AfterBean afterBean = after.get(i);
            Entry entry = new Entry(i, Float.parseFloat(afterBean.getFormaldehyde()));
            data2.add(entry);
        }
        if (data1.size() > 0) {
