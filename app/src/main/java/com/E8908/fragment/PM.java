...
public class PM extends RelativeLayout {
    ...
    private void init(Context context) {
        ...
        mManage = new ChartManage(mLineChart, context, 3);
    }
    public void setData(CurverBean.ResponseBean responseBean) {
        ...
        for (int i = 0; i < before.size(); i++) {
            ...
            Entry entry = new Entry(i, Float.parseFloat(beforeBean.getPm25()));
            data1.add(entry);
        }
        for (int i = 0; i < after.size(); i++) {
            CurverBean.ResponseBean.AfterBean afterBean = after.get(i);
            Entry entry = new Entry(i, Float.parseFloat(afterBean.getPm25()));
            data2.add(entry);
        }
        if (data1.size() > 0) {
