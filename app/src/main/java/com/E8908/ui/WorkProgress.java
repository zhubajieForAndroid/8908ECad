...
public class WorkProgress extends View{
    private int[] mColors = {Color.rgb(27,113,250),Color.rgb(33,202,245)};//进度条颜色（渐变色的2个点）
    private int backgroundColor = Color.BLACK;//进度条默认颜色
...
//  private RectF rect;//绘制范围
    ...
    public WorkProgress(Context context, AttributeSet attrs)
    {
    ...
    // 获得我自定义的样式属性
    public WorkProgress(Context context, AttributeSet attrs, int defStyle)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            switch (attr)
            {
                case R.styleable.GoodProgressView_startColor:
                    // 渐变色之起始颜色，默认设置为红色
                    mColors[0] = a.getColor(attr, Color.rgb(27,113,250));
                    break;
                case R.styleable.GoodProgressView_endColor:
                    // 渐变色之结束颜色，默认设置为品红
                    mColors[1] = a.getColor(attr, Color.rgb(33,202,245));
                    break;
                case R.styleable.GoodProgressView_backgroundColor:
                    // 进度条默认颜色，默认设置为灰色
                    backgroundColor = a.getColor(attr, Color.BLACK);
                    break;
...
//        mPaint.setStrokeWidth((float) lineWidth );
        mPaint.setStrokeWidth(GetSize.px2dip(getContext(),16));
