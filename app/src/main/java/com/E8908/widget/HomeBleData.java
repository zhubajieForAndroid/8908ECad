...
public class HomeBleData extends RelativeLayout implements View.OnClickListener {
    @Bind(R.id.start_link)
    Button mStartLink;
    ...
    @Bind(R.id.gas_container)
    LinearLayout mGasContainer;
    @Bind(R.id.bel_id)
    TextView mBelId;
    private Context mContext;

    public HomeBleData(Context context) {
        super(context);
        mContext = context;
        init();
    }
    ...
    private void init() {
        View.inflate(mContext, R.layout.fragment_home_ble_data_by_ble, this);
        ButterKnife.bind(this, this);
        mStartLink.setOnClickListener(this);
        //设置每个进度条的最大值
    ...
    public void setData(byte[] data, String bleID) {
        if (!TextUtils.isEmpty(bleID))
            mBelId.setText("蓝牙设备ID:"+bleID);
        if (data == null || data.length <= 0) {
            return;
        }

        String checkGasState = DataBleUtil.getBleCheckGasState(data);       //气体检测仪的状态
        String preheatState = checkGasState.substring(7, 8);  //预热状态
        if ("0".equals(preheatState)) {              //预热完成
            String tvoc = DataBleUtil.getTVOC(data);
            String formaldehyde = DataBleUtil.getFormaldehyde(data);//甲醛
            String pm = DataBleUtil.getPM(data);
            String humidity = DataBleUtil.getHumidity(data); //湿度
            String temperatureByCheck = DataBleUtil.getTemperatureByCheck(data);//温度
    ...
    public void setLinkBleState(boolean isLinkBle) {
        if (isLinkBle) {                         //蓝牙连接成功
            if (mSelectContainer.getVisibility() == View.VISIBLE) {
                mSelectContainer.setVisibility(View.GONE);
                mGasContainer.setVisibility(View.VISIBLE);
            }
        } else {
            if (mSelectContainer.getVisibility() == View.GONE) {
                mSelectContainer.setVisibility(View.VISIBLE);
                mGasContainer.setVisibility(View.GONE);
            }
        }
    }
    ...
    public void onClick(View v) {
        //连接蓝牙
        LinkBleDialog dialog = new LinkBleDialog(mContext, R.style.dialog);
        if (!dialog.isShowing())
            dialog.show();
    }
}
